...
export class GratitudeComponent implements OnInit {
  ...
  // Approve testimonial (for pending tab)
  async approveTestimonial(testimonialId: string): Promise<void> {
    try {
      // Here you would typically call an API to approve the testimonial
      // Since this endpoint is not in your service, I'm simulating the action
      
      console.log('Approving testimonial:', testimonialId);
      
      // Find and update the testimonial in the local array
      const testimonial = this.pendingTestimonials.find(t => t._id === testimonialId);
      if (testimonial) {
        testimonial.selected = true;
        
        // Move to received testimonials
        this.receivedTestimonials.unshift(testimonial);
        this.pendingTestimonials = this.pendingTestimonials.filter(t => t._id !== testimonialId);
      }
      
      await swalHelper.showToast('Testimonial approved successfully!', 'success');
      
    } catch (error) {
      console.error('Error approving testimonial:', error);
      await swalHelper.showToast('Failed to approve testimonial', 'error');
    }
  }

  // Reject testimonial (for pending tab)
  async rejectTestimonial(testimonialId: string): Promise<void> {
    try {
      // Here you would typically call an API to reject the testimonial
      console.log('Rejecting testimonial:', testimonialId);
      
      // Remove from pending testimonials
      this.pendingTestimonials = this.pendingTestimonials.filter(t => t._id !== testimonialId);
      
      await swalHelper.showToast('Testimonial rejected', 'info');
      
    } catch (error) {
      console.error('Error rejecting testimonial:', error);
      await swalHelper.showToast('Failed to reject testimonial', 'error');
    }
  }
