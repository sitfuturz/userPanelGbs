...
export class GratitudeService {
   ...
   */
  async getTestimonialsByReceiverId(receiverId: string, page: number = 1, limit: number = 10): Promise<TestimonialResponse> {
    try {
      this.getHeaders();
      ...
      await swalHelper.showToast(response.message || 'Testimonials fetched successfully', 'success');
   ...
   */
  async getTestimonialRequestsByReceiverId(receiverId: string, page: number = 1, limit: number = 10): Promise<TestimonialResponse> {
    try {
      this.getHeaders();
      ...
      await swalHelper.showToast(response.message || 'Testimonial requests fetched successfully', 'success');
      return response.data || response;
    } catch (error: any) {
  ...
  async createTestimonial(testimonialData: CreateTestimonialRequest): Promise<TestimonialResponse> {
    try {
      ...
      await swalHelper.showToast(response.message || 'Testimonial created successfully', 'success');
      return response.data || response;
    } catch (error: any) {
  ...
  async getAllUsersData(page: number = 1, limit: number = 50): Promise<UserResponse> {
    try {
      ...
      await swalHelper.showToast(response.message || 'Users data fetched successfully', 'success');
      return response.data || response;
    } catch (error: any) {
